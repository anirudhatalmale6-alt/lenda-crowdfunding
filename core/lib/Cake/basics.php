 ...
 */
    ...
    function getImageUrl($model, $attachment, $options)
    {
        ...
        $image_hash = $options['dimension'] . '/' . $model . '/' . $attachment['id'] . '.' . hash('sha256', Configure::read('Security.salt') . $model . $attachment['id'] . $options['type'] . $options['dimension'] . Configure::read('site.name') . Configure::read('site.version')) . '.' . $options['type'];
	...
	// Uses bcrypt (blowfish) as first choice, then SHA-512, SHA-256, or EXT_DES
	// MD5 and STD_DES have been removed for security reasons
	...
	function getCryptHash($str)
	{
		if (CRYPT_BLOWFISH) {
			...
			// Fallback to SHA-256 if no secure hashing available (MD5/STD_DES removed for security)
			$algo_selector = '$5$';
			$workload_factor = 'rounds=5000$';
