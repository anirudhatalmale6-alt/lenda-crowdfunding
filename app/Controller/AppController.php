...
class AppController extends Controller
{
    ...
    public function beforeFilter()
    {
        ...
        // CSRF Protection - allow API endpoints to bypass
        if (!$this->RequestHandler->prefers('json')) {
            $this->Security->csrfCheck = true;
        } else {
            // For API, handle CSRF via custom header
            $this->Security->csrfCheck = false;
        }
        
        // Set CSRF token in headers for API responses
        if ($this->RequestHandler->prefers('json')) {
            $this->response->header('X-CSRF-Token', $this->_getCsrfToken());
        }
    ...
    public function captcha_play($session_var = null)
    {
        ...
        $this->disableCache();
        // Use outputAudioFile method which is public and handles audio generation
     ...
     * @return bool
     */
    protected function _validateApiCsrfToken() {
