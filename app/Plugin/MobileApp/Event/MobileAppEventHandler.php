 ...
 */
class MobileAppEventHandler extends CakeObject implements CakeEventListener
{
    /**
     * implementedEvents
     *
    ...
    public function validate_user($event) 
    {
        ...
        if ($obj->Auth->login()) {
            $mobile_app_hash = hash('sha256', $obj->_unum() . $obj->request->data['User'][Configure::read('user.using_to_login') ] . $obj->request->data['User']['password'] . Configure::read('Security.salt'));
    ...
    public function saveiPhoneAppThumb($attachments, $model = 'Project') 
    {
        ...
        foreach($options as $option) {
            if (!empty($attachment['id'])) {
                $destination = APP . 'webroot' . DS . 'img' . DS . $option['dimension'] . DS . $model . DS . $attachment['id'] . '.' . hash('sha256', Configure::read('Security.salt') . $model . $attachment['id'] . $option['type'] . $option['dimension'] . Configure::read('site.name')) . '.' . $option['type'];
